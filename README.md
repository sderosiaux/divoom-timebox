...
```bash
./install-agent.sh install logocycle --each 60 --fade 0.8
./install-agent.sh log            # follow it
./install-agent.sh stop           # give the panel back
./install-agent.sh uninstall      # remove the agent entirely
```

Anything after `install` goes straight to `python -m divoom.serve`:
...
Two files, neither versioned because both carry absolute paths:

```
~/Library/LaunchAgents/io.conduktor.divoom.plist   generated by the script
~/Library/Logs/io.conduktor.divoom.log             connections, losses, uptime
```

launchd owns the process, so it outlives the terminal that started it and comes
back at login. That covers the interpreter dying. It does not cover the two
failures that actually happen, because neither kills the process: the speaker
...
resetting once a run has held for a minute, so that a failure after six good
hours does not inherit a two-minute wait.

Sleeping the Mac suspends the process and powers down Bluetooth. The panel keeps
showing its last frame, since the device has no idea the stream ended. On wake
the first write fails and the loop reconnects, about ten seconds.

Verified: killed with `SIGKILL`, launchd brought it back and it reconnected in
three seconds. Killed the bridge alone, leaving the Python process alive: it
logged `Broken pipe` and recovered in eight. A flat battery, met by accident,
surfaces as `kIOReturnTimeout` and retries. Login persistence rests on
`RunAtLoad` and has not been tested across a reboot.

The device exposes no battery query, so the log is the only way to learn how long
it lasts unplugged: charge it, pull the cable, and read the `lost after Ns` line.
A sleeping Mac produces the same line, so check whether a reconnection follows.
